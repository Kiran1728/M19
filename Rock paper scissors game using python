...
class RpsGame():
    ...
    def player(self):
        if self.rock_btn.clicked((30, 520)):
            self.p_option = "rock"
            self.screen.blit(self.choose_rock, (120, 200))
        elif self.paper_btn.clicked((340, 520)):
            self.p_option = "paper"
            self.screen.blit(self.choose_paper, (120, 200))
        else:
            self.scissors_btn.clicked((640, 520))
        ...
        while run:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.rock_btn.clicked((30, 520)) or self.paper_btn.clicked((340, 520)) or self.scissors_btn.clicked((640, 520)):
                        self.image_reset()
                        self.player()
                        self.computer()
                        self.score_cache()
